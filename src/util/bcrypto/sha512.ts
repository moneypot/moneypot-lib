...
export default class SHA512 {
  ...
  _update(data: Uint8Array, len: number) {
  ...
  prepare(chunk: Uint8Array, pos: number) {
    const W = this.msg;
...
function sum64(buf: Uint32Array, pos: number, ah: number, al: number) {
  const bh = buf[pos];
  const bl = buf[pos + 1];
...
function sum64_hi(ah: number, al: number, bh: number, bl: number) {
  const lo = (al + bl) >>> 0;
  const hi = (lo < al ? 1 : 0) + ah + bh;
  return hi >>> 0;
}

function sum64_lo(ah: number, al: number, bh: number, bl: number) {
  const lo = al + bl;
  return lo >>> 0;
}
...
function rotr64_hi(ah: number, al: number, num: number) {
...
function shr64_hi(ah: number, al: number, num: number) {
  return ah >>> num;
}
...
function ch64_hi(xh: number, xl: number, yh: number, yl: number, zh: number) {
  let r = (xh & yh) ^ ((~xh) & zh);
...
function ch64_lo(xh: number, xl: number, yh: number, yl: number, zh: number, zl: number) {
  let r = (xl & yl) ^ ((~xl) & zl);
...
function maj64_hi(xh: number, xl: number, yh: number, yl: number, zh: number) {
...
function maj64_lo(xh: number, xl: number, yh: number, yl: number, zh: number, zl: number) {
